...
export function HomeCafeSection() {
  ...
  return (
    <section className="mt-10">
      ...
      <ul className="mt-5 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6">
        {list.map((item, idx) => (
          <li key={`fav-${item.id}`}>
            <Link
              href={`/cafes/${item.id}`}
              className={`
                ...
                p-5 h-60 flex items-center justify-center hover:bg-secondary duration-150`}
            >
              <Image src={item.icon} alt={item.title} height={128} width={128} />
            </Link>
          </li>
        ))}
      </ul>
